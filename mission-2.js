...
function select_datasheet()
{
    ...
    for (let i = 1; i <= 2; i++)
    {
        const select_unbound = document.getElementById("datasheet-selector-" + i);
        if (select_unbound.value !== "none")
        {
            selected_datasheet.push(select_unbound.value);
 
        }
        localStorage.setItem("m"+mission_num+"-select-datasheet-" + i, select_unbound.value);

    }
    const selection = document.querySelector("." + event.target.value);
    if (selected_datasheet.length > 0)
    {
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    for (let i = 1; i <= 2; i++)
    {
        const datasheet = document.getElementById("datasheet-selector-" + i);
        datasheet.addEventListener("change", (_) => {select_datasheet()});
        datasheet.value = localStorage.getItem("m"+mission_num+"-select-datasheet-"+i) || "none";
    }
    select_datasheet();
